...
const AdminCategoryBanners = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const banRes = await fetch(`${API_BASE_URL}/api/category-banners`);
      ...
      setSelectedCategory(prev => prev || predefinedCategories[0]);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/category-banners`, {
        method: 'POST',
  ...
  const handleDelete = async (categoryName) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/category-banners/${encodeURIComponent(categoryName)}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${user.token}` }
      });
      if (res.ok) {
